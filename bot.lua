			...
			if is_naji(msg) then
				if text:match("^(افزودن مدیر) (%d+)$") then
					...
					if redis:sismember('botBOT-IDadmin', matches) then
						return send(msg.chat_id_, msg.id_, "کاربر مورد نظر در حال حاضر مدیر است.")
					elseif redis:sismember('botBOT-IDmod', msg.sender_user_id_) then
						...
						return send(msg.chat_id_, msg.id_, "مقام کاربر به مدیر ارتقا یافت")
					end
				...
				elseif text:match("^همگام سازی با تبچی$") then
					...
					return send(msg.chat_id_, msg.id_, "همگام سازی اطلاعات با تبچی شماره "..tostring(botid).." انجام شد.")
				...
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"تازه‌سازی آمار تبلیغ‌گر شماره  BOT-ID  با موفقیت انجام شد.")
